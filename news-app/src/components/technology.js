...
const Technology = () => {
  ...
  const fetchTopNewsWithImages = async () => {
    try {
        const apiUrl = `https://newsapi.org/v2/top-headlines?country=in&category=technology&pageSize=20&apiKey=${apiKey}`;
  ...
  const fetchTechnologyNewsQuery = async (query) => {
    try {
      const apiUrl = `https://newsapi.org/v2/top-headlines?q=${query}&country=in&category=technology&pageSize=20&apiKey=${apiKey}`;
      const response = await axios.get(apiUrl);
      const articlesWithImages = response.data.articles.filter(article => article.urlToImage !== null);
      setArticles(articlesWithImages.slice(0, 10));
    } catch (error) {
  ...
  const handleSearch = () => {
    if (query.trim() !== '') {
      fetchTechnologyNewsQuery(query);
    }
  };
  ...
  return (
    <div className="business-page" style={{ backgroundColor: 'rgb(33, 55, 100)', minHeight: '100vh' }}>
      <header>
        <nav className="navbar">
          <div className="logo">Business News.</div>
