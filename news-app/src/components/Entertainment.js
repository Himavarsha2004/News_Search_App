...
const Entertainment = () => {
  ...
  const fetchTopNewsWithImages = async () => {
    try {
        const apiUrl = `https://newsapi.org/v2/top-headlines?country=in&category=entertainment&pageSize=20&apiKey=${apiKey}`;
  ...
  const fetchEntertainmentNewsQuery = async (query) => {
    try {
        const apiUrl = `https://newsapi.org/v2/top-headlines?q=${query}&country=in&category=entertainment&pageSize=20&apiKey=${apiKey}`;
      const response = await axios.get(apiUrl);
      const articlesWithImages = response.data.articles.filter(article => article.urlToImage !== null);
      setArticles(articlesWithImages.slice(0, 10));
    } catch (error) {
  ...
  const handleSearch = () => {
    if (query.trim() !== '') {
      fetchEntertainmentNewsQuery(query);
    }
  };
  ...
  return (
    <div className="entertainment-page" style={{ backgroundColor: 'rgb(33, 55, 100)', minHeight: '100vh' }}>
      <header>
        <nav className="navbar">
          <div className="logo">Entertainment News.</div>
