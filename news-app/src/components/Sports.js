...
const Sports = () => {
  ...
  const fetchTopNewsWithImages = async () => {
    try {
        const apiUrl = `https://newsapi.org/v2/top-headlines?country=in&category=sports&pageSize=20&apiKey=${apiKey}`;
  ...
  const fetchSportsNewsQuery = async (query) => {
    try {
        const apiUrl = `https://newsapi.org/v2/top-headlines?q=${query}&country=in&category=sports&pageSize=20&apiKey=${apiKey}`;
      const response = await axios.get(apiUrl);
      const articlesWithImages = response.data.articles.filter(article => article.urlToImage !== null);
      setArticles(articlesWithImages.slice(0, 10));
    } catch (error) {
  ...
  return (
    <div className="sports-page" style={{ backgroundColor: 'rgb(33, 55, 100)', minHeight: '100vh' }}>
      <header>
        <nav className="navbar">
          <div className="logo">Sports News.</div>
