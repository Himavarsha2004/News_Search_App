...
const Business = () => {
  ...
  const fetchTopNewsWithImages = async () => {
    try {
      const apiUrl = `https://newsapi.org/v2/top-headlines?country=in&category=business&pageSize=20&apiKey=${apiKey}`;
  ...
  const fetchBusinessNewsQuery = async (query) => {
    try {
      const apiUrl = `https://newsapi.org/v2/top-headlines?q=${query}&country=in&category=business&pageSize=20&apiKey=${apiKey}`;
      const response = await axios.get(apiUrl);
      const articlesWithImages = response.data.articles.filter(article => article.urlToImage !== null);
      setArticles(articlesWithImages.slice(0, 10));
    } catch (error) {
